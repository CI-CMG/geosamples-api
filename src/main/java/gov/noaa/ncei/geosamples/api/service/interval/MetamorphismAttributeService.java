...
@Component
class MetamorphismAttributeService extends AttributeService<CuratorsWeathMetaEntity> {

  @Autowired
  MetamorphismAttributeService(CuratorsWeathMetaRepository repository, WeathMetaSpecificationFactory specificationFactory) {
  ...
  protected SpecExtender<CuratorsWeathMetaEntity> getAdditionalSpecifications() {
    return (specs, r, cb, j) -> {
      ...
      specs.add(SearchUtils.startsWithIgnoreCase(cb, "metamorphism - ", r.get(CuratorsWeathMetaEntity_.WEATH_META)));
    };
  }

  @Override
  protected String mapItem(String item) {
    return item.replaceFirst("metamorphism - ", "");
  }
}
